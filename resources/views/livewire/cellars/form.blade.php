            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
              <form wire:submit.prevent="save" class="py-10 px-4" novalidate>
                <div>
                    ...
                    <x-jet-input class="block mt-1 w-full" type="text" wire:model="cellar.name" autofocus />
                    @error('cellar.name') <span class="error" style="color: red;">{{ $message }}</span> @enderror
                </div>

                <div class="mt-4">
                  <x-jet-label value="Ubicación" />
                  <x-jet-input class="block mt-1 w-full" wire:model="cellar.ubication" type="text" />
                  @error('cellar.ubication') <span class="error" style="color: red;">{{ $message }}</span> @enderror
                </div>
